package dev.gabbo.kitpvpreforged.listeners;

import dev.gabbo.kitpvpreforged.KitPvP;
import dev.gabbo.kitpvpreforged.events.BountyClaimEvent;
import dev.gabbo.kitpvpreforged.data.PlayerData;
import dev.gabbo.kitpvpreforged.events.KillStreakEvent;
...
public class PlayerListener implements Listener {
    ...
    public void handleDeath(PlayerDeathEvent event) {
        ...
        if (data.lastPlayer != null) {
            ...
            if (!data.lastPlayer.equals(killed)) {
                ...
                if (killerData.streak % streak == 0) Bukkit.getPluginManager().callEvent(new KillStreakEvent(data.lastPlayer, killerData.streak));
            }
        ...
        } else killed.sendMessage(ChatUtils.getFormattedText("kills.default-death").replaceAll("%player%", killed.getName()));
    ...
    public void onBountyClaim(PlayerDeathEvent event) {
        ...
        if (data.bounty >= 5000) Bukkit.getOnlinePlayers().forEach(player -> player.playSound(player.getLocation(), Sound.WITHER_SPAWN, 4, 1));
    ...
    public void onPlayerPickup(PlayerPickupItemEvent event) {
        ...
        if (item.getType().equals(Material.GOLDEN_APPLE) && !item.isSimilar(new ItemStack(Material.GOLDEN_APPLE, item.getAmount(), (short) 1)) && !data.pickupGoldenApple) event.setCancelled(true);
